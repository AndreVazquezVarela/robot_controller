...
class QLearningTurtlebot:
    def __init__(self):
        rospy.init_node('q_learning_turtlebot')
        ...
        self.agent = QLearningAgent(self.action_space, self.state_size)
    ...
    def laser_callback(self, laser_msg):
        ...
        if self.current_state is not None and self.next_state is not None:
            ...
            self.agent.update_q_table(self.current_state, action, reward, self.next_state)
    ...
    def run(self, num_epochs):
        for epoch in range(num_epochs):
            ...
            f = open("q-learning.txt", "a")
            f.write(str(self.total_reward) + ",")
            f.close()
        self.agent.save_model('qlearning_model.pkl')
...
if __name__ == '__main__':
    try:
        q_learning_turtlebot = QLearningTurtlebot()
        q_learning_turtlebot.run(num_epochs=50)  # Ejemplo: entrenar durante 10 épocas
